...
public class GameOfMonopoly {
  ...
  public void buyProperty(Player player) throws InvalidMove {
    ...
    if (!(loc instanceof Property)) {
      throw new InvalidMove(player + " cannot buy location " + loc.getName() + ": it's not a property!");
    }
    Property prop = (Property) loc;
    if (prop.hasOwner()) {
      throw new InvalidMove(
          player + " cannot buy location " + prop.getName() + ": it's owned by " + prop.getOwner().getName() + "!");
    }
    if (prop.getPrice() > player.getBalance()) {
      throw new InvalidMove(player + " cannot buy location " + prop.getName() + ": player has insufficient funds!");
    }
  ...
  public void sellProperty(Player player, Location loc) throws InvalidMove {
    if (!(loc instanceof Property)) {
      throw new InvalidMove(player + " cannot sell location " + loc.getName() + ": it's not a property!");
    }
    Property prop = (Property) loc;
  ...
  public void mortgageProperty(Player player, Location loc) throws InvalidMove {
    if (!(loc instanceof Property)) {
      throw new InvalidMove(player + " cannot mortgage location " + loc.getName() + ": it's not a property!");
    }
    Property prop = (Property) loc;
    if (prop.getOwner() != player) {
      throw new InvalidMove(player + " cannot mortgage location " + loc.getName() + ": it's not theirs!");
    }
    if (prop.isMortgaged()) {
      throw new InvalidMove(player + " cannot mortgage location " + loc.getName() + ": it's already mortgaged!");
    }
  ...
  public void unmortgageProperty(Player player, Location loc) throws InvalidMove {
    if (!(loc instanceof Property)) {
      throw new InvalidMove(player + " cannot unmortgage location " + loc.getName() + ": it's not a property!");
    }
    Property prop = (Property) loc;
    if (prop.getOwner() != player) {
      throw new InvalidMove(player + " cannot unmortgage location " + loc.getName() + ": it's not theirs!");
    }
    if (!prop.isMortgaged()) {
      throw new InvalidMove(player + " cannot mortgage location " + loc.getName() + ": it's not mortgaged!");
    }
    int cost = (int) ((double) (prop.getPrice() / 2) * 1.1);
    if (cost > player.getBalance()) {
      throw new InvalidMove(player + " cannot mortgage location " + loc.getName() + ": insufficient funds!");
    }
